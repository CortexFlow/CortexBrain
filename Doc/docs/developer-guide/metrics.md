...
CortexBrain's metrics come in three tiers, documented below:

1. **Live eBPF / gRPC metrics** - currently integrated on the `main` branch, exposed via the `agent.Agent` gRPC service.
2. **OpenTelemetry metrics** - the incoming metric suite (16 instruments) on the `origin/metrics-patch` branch, exported via OTLP.
3. **Deprecated proxy metrics** - the old Prometheus `/metrics` endpoint from the now-removed proxy-sidecar component, kept only as a historical reference.

See the [Agent API Overview](agent-api.md) for how the gRPC methods map to the BPF maps behind these metrics.

## Live eBPF / gRPC metrics

These metrics are collected by eBPF perf buffers and surfaced through three of the `agent.Agent` RPC methods. There is no Prometheus endpoint on the current branch - the data is pulled via `cfcli` or any tonic gRPC client.
...
**Source**: `core/api/protos/agent.proto` (`LatencyMetric`, `LatencyMetricsResponse`), `core/src/components/metrics_tracer/src/main.rs` (kprobes), `core/api/src/api.rs` (handler at line 564).
...
**Source**: `core/api/protos/agent.proto` (`DroppedPacketMetric`, `DroppedPacketsResponse`), `core/src/components/metrics_tracer/src/main.rs` (kprobe + offsets), `core/api/src/api.rs` (handler at line 627).

**CLI**: `cfcli monitoring droppedpackets`

## OpenTelemetry metrics (incoming - `metrics-patch`)

!!! warning "Not yet merged to `main`
    The OpenTelemetry metric suite described in this section lives on the `origin/metrics-patch` and `origin/cpu-metrics` branches. It is documented here so that contributors know what is coming, but it is not yet available on the default branch.

The `metrics-patch` branch adds a full OpenTelemetry pipeline:
...
The `cpu_idle_state` instrument additionally carries a `cpu_id` attribute.
...
### New eBPF probes feeding these instruments

The `metrics-patch` branch adds these eBPF probes (`core/src/components/metrics_tracer/src/`):
...
## Deprecated proxy metrics

The old proxy-sidecar component exposed an HTTP `/metrics` endpoint in Prometheus text format on port 9090. That component has been **removed** from the codebase; the metric definitions survive only as captured output in the [March 2025 Test Report](../test-report/reports/March2025/March2025.md).

| Name | Type | Labels |
|------|------|--------|
| `dns_response_time` | histogram | `server` (`service_discovery`, `service_discovery_tcp`, `service_discovery_udp`) |
| `total_dns_requests` | counter | `client_ip` |

These are documented for historical context only and are not part of the current codebase.